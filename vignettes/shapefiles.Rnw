...
\texttt{GDAL} and \pkg{sp} uses \pkg{sf} under the hood.
...
projections available. It may be wise to use \pkg{sf}
...
\subsubsection{Geometries of class \texttt{MULTILINESTRING}}
...
In the \pkg{sf} package, a geometry column of class \texttt{MULTILINESTRING}
...
The list-of-lists hierarchy in a \texttt{MULTILINESTRING} object 
is useful when representing internal divisions in a country.
For example, if \texttt{USA} is an object of class \texttt{sf} with geometry column of class \texttt{MULTILINESTRING}
representing the borders of the United States
of America, then \texttt{USA} have 52 rows representing the 52 states. The geometry column \verb!geo <- st_geometry(USA)! is then a list of length 52, where \verb!geo[[i]]! might represent the borders of the \texttt{i}-th State. 
The borders of each State consist of several different curved lines. Thus 
\verb!geo[[i]][[j]]! would represent the \texttt{j}th 
piece of the boundary of the \texttt{i}-th State and would be of class \texttt{LINESTRING}.
