...
% $Revision: 1.3 $ $Date: 2023/08/16 02:16:49 $
...
\texttt{From help files:}

Low-level functions which calculate the estimated K function and
     estimated pair correlation function (or any similar functions)
...
\begin{verbatim}
      D: A square matrix giving the distances between all pairs of
          points.
