...
\section{\pkg{spatstat} is splitting into parts}
    ...
    Currently there are four sub-packages, called 
    \pkg{spatstat.utils},
    \pkg{spatstat.data},
    \pkg{spatstat.sparse},
    and \pkg{spatstat}.
    ...
    \begin{itemize}
    \item 
      The \code{spatstat} package contains the main code.
    \item 
    ...
    will automatically load \pkg{spatstat.data}
    and silently \textbf{``import''}
