...
\begin{itemize}
    ...
    \item \texttt{uniquemap.data.frame}:
...
%%B
 ...
 \item \texttt{bw.ppl}:
  ...
  \item \texttt{bw.scott}: the two bandwidth values in the result
...
\item \texttt{latest.news}: 
 ...
 \item \texttt{lohboot}:
  ...
  \item 
    ...
    \texttt{Lcross}, \texttt{Lcross.inhom} are now computed differently.
    First a confidence band is computed for the corresponding $K$
    function \texttt{Kest}, \texttt{Kinhom}, \texttt{Kcross},
