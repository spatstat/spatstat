...
% $Revision: 1.93 $ $Date: 2026/06/01 23:41:23 $
...
\date{\spst\ version \Sexpr{sversion}, \Sexpr{outed} on \Sexpr{sdate}}
\maketitle
...
\section{Pr{\'e}cis of all changes}

Here is the text from the `overview' sections of 
the News and Release Notes for each update. 

\begin{itemize}
\item Variance-covariance matrix for determinantal models.
\item More diagnostics for determinantal models.
\item Added variable plots for all kinds of models.
\item Adaptive kernel smoothing of mark values.
\item Random Dirichlet tessellation avoiding edge effects.
\item More capabilities for three-dimensional point patterns.
\item More methods for \texttt{intensity}
\item More methods for repulsion index \texttt{repul}.
\item Changed rule for converting bandwidths in pair correlation.
\item Improved handling of boundary pixels in \texttt{density()}, 
  \texttt{relrisk()}, \texttt{Smooth()}
\item Improved handling of boundary pixels in model predictions.
\item Improvements to rose diagrams.
\item Fiksel's bandwidth rule.
\item Faster calculation of Ripley's isotropic edge correction.
\item Random diffusion of points.
\item Random unrounding.
\item Truncated Poisson distribution.  
\item Improved handling of boundary pixels 
  when discretising a polygonal window.
\item Improved handling of `rejected' points.
\item Simulate a Markov chain with sparse transition matrix.

\item  Weighted sum of outer products in a 3D array or sparse array.
  
\item Distribution of a weighted sum of chi-squared random variables.
  
\item New generic \texttt{Lmodel} computes the theoretical $L$-function
  of a fitted model or theoretical model.

\item \texttt{envelope} method for determinantal point process models.

\item Specify the theoretical value of the summary function 
  in \texttt{envelope}.
  
\item Important changes to default settings for pair correlation functions.
  
\item More flexible interpretation of arguments \texttt{lambda = <ppm>}
  or \texttt{lambda = <lppm>}
  in inhomogeneous summary statistics

\item New options for multitype pair correlation functions.

\item More options for \texttt{simulate} argument in \texttt{envelope} methods.

\item Extended capability of \texttt{envelope.pp3}

\item Method of T.F. Cox for identifying dense and sparse regions of points.
  
\item New class of (theoretical) cluster process models

\item Inhomogeneous Poisson process in 3D.

\item Non-uniform random points in 3D.
    
\item Simple Sequential Inhibition in 3D.
  
\item Support function, Feret diameter, polar body, Voronoi flower.

\item Merge tiles in a tessellation.

\item Return squared nearest neighbour distances.
    
\item Bug fix in Minkowski sum.

\item Summary method for tessellations.
  

\item Identify threads in a network

\item Improved algorithm for integrating a function over a network.

\item Summary functions have argument \texttt{rmax}.
  
\item Diagnostic functions have argument \texttt{rmax}.
  
\item More support for long vectors (vectors with more than $2^{31}$ entries).
  
\item Waagepetersen's composite likelihood for cluster processes.  
  
\item Important changes to mark correlation function.
  
\item Convert a recursively partitioned point process model
  to a spatial tessellation.
  
\item Signed distance transform and signed distance function.
  
\item Spatial persistence index and cluster strength index.
  
\item Improvements to \texttt{profilepl} and \texttt{effectfun}.

\item Improvements to perspective plot of a function on a network.
  
\item Replicated point patterns on a linear network.
  
\item Diagnostics for a fitted point process model on a linear network.
  
\item \texttt{lppm} recognises local coordinates in the model formula.
  
\item Major extension of code for ROC curves.  

\item ROC curves for determinantal point process models.
    
\item Extended facilities for spatial logistic regression.

\item Level sets calculated using analytic geometry, for some functions.

\item Easier control over transparency of image plots.

\item Scramble the colours when plotting a point pattern 
  or line segment pattern.
  
\item Different discretisation rules supported in \texttt{slrm}.

\item Leave-one-pixel-out prediction in \texttt{slrm}.
  
\item Generate Gaussian random fields.

\item Lurking variable plot on a network.

\item Partial residual plot on a network.

\item Quadrat counting and quadrat tests on a linear network.

\item More geometrical operations on linear network objects.

\item Pixelwise arithmetic on a linear network.
  
\item Partial residuals for cluster process and Cox process models.

\item Extended facilities for spatial logistic regression.

\item Improvements to perspective views of images.

\item Missing or unavailable (NA) spatial objects
  are accepted by \texttt{solist} and \texttt{hyperframe}.
  
\item Missing or unavailable (NA) spatial objects
  are handled by \texttt{solapply}, \texttt{with.hyperframe}
  and by many low-level functions.

\item Summary functions handle NA objects.

\item Simulation of a fitted model in a different window is better supported.  
  
\item More options when simulating a fixed number of random points.  
  
\item \texttt{unnormdensity} returns the smoothing parameters used.  
  
\item Find centroids of tiles of a tessellation.

\item More options for converting a tessellation to an image.
  
\item Youden index.
  
  \item Shrinkage estimator in \texttt{Smooth.ppp}.

  \item Backgrounds for plots of point patterns.
  \item Connected components of polygonal windows.
  \item More options for disc windows.

  \item Interactively identify tiles of a tessellation.
  \item Interactively identify segments of a network.
  \item Interactively identify tiles of a tessellation on a network.
  \item \texttt{spatstatLocator} supports `snap' behaviour (rounding the
    clicked location to the nearest grid point). 
  \item \texttt{clickpoly} allows vertices to be `snapped' (rounded to
    the nearest grid point).

  \item Corrected unit of length in two datasets.
    
\item New options for defining nearest neighbours.

\item \texttt{im.apply} handles vector-valued functions.

\item Remove small holes from a window.
  
  \item Extension to weighted median and weighted quantiles.
...
\begin{itemize}
\item \texttt{clusterprocess}:
  Neyman-Scott-Cox cluster process
...
\begin{itemize}
\item \texttt{auc.dppm}:
  Area under the ROC curve for a fitted determinantal point process model.
  Method for \texttt{auc} for objects of class \texttt{dppm}.
\item \texttt{parres.dppm}:
  Partial residuals for a fitted determinantal point process model.
  Method for \texttt{parres} for objects of class \texttt{dppm}.
\item \texttt{vcov.dppm}:
  Variance-covariance matrix for the trend parameters in a
  fitted determinantal point process model.
\item \texttt{SmoothAdaptiveKernel.ppp}:
    Adaptive kernel smoothing of mark values of a point pattern.
\item \texttt{rpoisDirichletTess}:
    Dirichlet tessellation induced by a Poisson process on the infinite
    two-dimensional plane, avoiding edge effects.
\item \texttt{distbdry}: New generic.
\item \texttt{distbdry.ppp}: 
  distance from each point in a 2D point pattern
  to the boundary of the observation window. 
  Equivalent to \texttt{bdist.points}.
\item \texttt{distbdry.pp3}:
  distance from each point in a 3D point pattern
  to the boundary of the containing box. 
\item \texttt{distbdry.lpp}:
  distance from each data point on a linear network
  to the nearest terminal vertex.
\item \texttt{rjitter.pp3}:
  randomly displace the points in a 3D point pattern.
\item \texttt{intensity.kppm}: Intensity of a fitted cluster process
  or Cox process model.
\item \texttt{intensity.lppm}: Intensity of a fitted point process model
  on a linear network.
\item \texttt{intensity.mppm}: Intensity of a point process model
  fitted to multiple point patterns.
\item \texttt{repul.detpointprocfamily}:
  Repulsion index for determinantal point process model.
\item \texttt{repul.kppm}, \texttt{repul.clusterprocess}:
  Repulsion index for cluster process models and Cox models
  (gives negative value).
\item \texttt{repul.slrm}, \texttt{repul.rppm}:
  Repulsion index for spatial logistic regression models
  and recursive partition models (gives zero value)
\item \texttt{repul.ppm}, \texttt{repul.mppm}:
  Repulsion index for fitted Poisson or Gibbs models
  using Poisson-saddlepoint approximation.
\item \texttt{bw.fiksel}: Fiksel's (1988) rule for smoothing bandwidth for
    estimating the pair correlation function.
\item \texttt{rdiffuse}:
  Randomly perturb each point in a point pattern by diffusion.
\item \texttt{rUnround}, \texttt{rUnround.ppp}
  Random un-rounding of a discretised point pattern.
\item \texttt{dpoisnonzero}, \texttt{ppoisnonzero}, \texttt{qpoisnonzero}:
  Density, cumulative probability and quantile function for
  a Poisson random variable conditioned to be nonzero.
\item \texttt{dpoistrunc}, \texttt{ppoistrunc}, \texttt{qpoistrunc}
  Density, cumulative probability and quantile function for
  the truncated Poisson distribution.
\item \texttt{runSparseMarkovChain} (in \texttt{spatstat.sparse}):
...
\item \texttt{sumsymouter} (in \texttt{spatstat.sparse}):
  Weighted sum of outer products in a 3D array or sparse array.
\item \texttt{length.sparse3Darray}:
  method for \texttt{length} for sparse 3D arrays.
\item \texttt{dsocs}, \texttt{psocs}, \texttt{qsocs}, \texttt{rsocs}:
  ...
  \item \texttt{farebro}: Farebrother's Algorithm 
    for the cumulative probability of
    a weighted sum of non-central chi-squared variables.
\item \texttt{envelope.dppm}:
  Method for \texttt{envelope} for fitted models of class \texttt{"dppm"}
\item \texttt{Lmodel}, \texttt{Lmodel.kppm}, \texttt{Lmodel.dppm}, 
  \texttt{Lmodel.ppm}, \texttt{Lmodel.detpointprocfamily}:
  Given a point process model, return a function in the \textsl{R} language
  which computes the theoretical $L$-function of the model.
\item \texttt{Kmodel.clustermodel}, \texttt{pcfmodel.clustermodel}, 
  \texttt{Lmodel.clustermodel}:
    Calculate the theoretical $K$-function, $L$-function or pair correlation
    function for a model of class \texttt{"clustermodel"}.
\item \texttt{coxmap}:
...
\item \texttt{clusterprocess}:
  Create an object of class \texttt{clusterprocess}.
\item \texttt{clusterradius.clusterprocess}:
  Radius of the support of the offspring density of a cluster process model.
\item \texttt{predict.clusterprocess}:
  Calculate intensity of a cluster process model
\item \texttt{print.clusterprocess}:
  Print a cluster process model
\item \texttt{reach.clusterprocess}:
  Distance beyond which the pair correlation is effectively 1.
\item \texttt{simulate.clusterprocess}
    Generate simulated realisations of a cluster process model.
\item \texttt{mergeTiles}: Merge tiles in a tessellation.
\item \texttt{mergeTiles.lintess}: Merge tiles in a tessellation on a network.
\item \texttt{FeretDiamFun}:
...
\item \texttt{minFeretDiam}, \texttt{maxFeretDiam}:
  Minimum or maximum Feret diameter of a domain
\item \texttt{SupportFun}
  Create a function that computes the support function of a domain.
\item \texttt{polarbody}
  Compute the polar body of a domain.
\item \texttt{voronoiFlower}
  Compute the Voronoi flower of a domain.
\item \texttt{summary.tess}, \texttt{print.summary.tess}
    Method for \texttt{summary} for tessellations.
\item \texttt{capitalise}: make the first letter of each word a capital letter.
\item \texttt{default.colourmap}: default colour map for a vector or factor.
\item \texttt{levelset}: new generic
\item \texttt{levelset.im}: level set of a pixel image
\item \texttt{levelset.distfun}: level set of a distance function.
\item \texttt{levelset.tessfun}: level set of a function created by
  \texttt{as.function.tess}.
\item \texttt{difflong}: lagged difference of long vector.
\item \texttt{persist}: spatial persistence index for a cluster process model.
\item \texttt{clusterstrength}: cluster strength index for
  a cluster process or Cox process model.
\item \texttt{as.tess.rppm}: convert a recursively partitioned 
  point process model to a spatial tessellation.
\item \texttt{formula.rppm}: extract the model formula
  of a recursively partitioned point process model.
\item \texttt{parres}: new generic function for partial residual plots.
\item \texttt{parres.ppm}: the original function \texttt{parres}
  has become the method \texttt{parres.ppm}.
\item \texttt{parres.kppm}: partial residual plot for
  fitted cluster process or Cox process models.
\item \texttt{diagnose}: new generic function for model diagnostics.
\item \texttt{diagnose.lppm}:
  diagnostic plots for point process model on a linear network.
\item \texttt{parres.lppm}:
  partial residual plot for point process model on a linear network.
\item \texttt{lurking.lppm}:
  Lurking variable plot for point process model on a linear network.
\item \texttt{lurking.lpp}:
  Lurking variable plot for point pattern on a linear network.
\item \texttt{residuals.lppm}:
  residual measure for point process model on a linear network.
\item \texttt{eem.lppm}:
  Stoyan-Grabarnik exponential energy weights diagnostic
  for point process model on a linear network.
\item \texttt{flipxy.linnet}:
  Swap $x$ and $y$ coordinates of a linear network
\item \texttt{flipxy.lpp}:
  Swap $x$ and $y$ coordinates of a point pattern on a linear network
\item \texttt{as.lintess}:
  Convert other kinds of data to a tessellation on a network.
\item \texttt{quadratcount.lpp}
  Method for quadrat counting on a linear network.
\item \texttt{quadrat.test.lpp}, \texttt{quadrat.test.lppm}
  Methods for quadrat test on a linear network.
\item \texttt{harmonise.linim}:
  Convert several images on a linear network to a common pixel grid.
\item \texttt{linim.apply}:
  Apply a function pixelwise to a list of images on a network.
\item \texttt{roc.dppm}: ROC curve for determinantal point process model.
\item \texttt{markequal}:     
  Mark equality function $e(r)$ for multitype point patterns.
\item \texttt{nnequal}, \texttt{nncount}:
  Summary functions for multitype point patterns
    based on the types of nearest neighbours.
\item \texttt{tolcon}: Tolerance contours for spatial relative risk.
\item \texttt{rGRFexpo}, \texttt{rGRFgauss}, \texttt{rGRFmatern}, 
  \texttt{rGRFstable}, \texttt{rGRFgencauchy}:
  generate a Gaussian random field.
\item \texttt{tile.centroids}:     
...
\item \texttt{dropROC}:
  ...
  \item \texttt{dropply}, \texttt{addapply}:
    Consider all single-variable additions or deletions in a model.
\item \texttt{roc.rhohat}, \texttt{roc.cdftest}, \texttt{roc.bermantest}:
  calculate ROC curve from other kinds of data.
\item \texttt{roc.im}: calculate ROC curve from a pixel image
  such as an estimate of intensity.
\item \texttt{youden}: Youden statistic for ROC curves.
    
\item \texttt{fillholes.owin}: remove small holes from a window.
\item \texttt{NAobject}: create a missing object of a particular class.
\item \texttt{is.NAobject}: recognise whether an object is a missing object.
\item \texttt{is.na.hyperframe}: determine which entries in a hyperframe
  are missing (NA values or NA objects).
\item \texttt{as.ppp.NAobject}, \texttt{as.owin.NAobject}, 
  \texttt{as.im.NAobject}, \texttt{domain.NAobject}, \texttt{Frame.NAobject}, 
  \texttt{Window.NAobject}, \texttt{marks.NAobject}, \texttt{marks<-.NAobject}:
  methods for the generics \texttt{as.ppp}, \texttt{as.owin} etc, 
  which return an \texttt{NAobject}.
  \item \texttt{npoints.NAobject}, \texttt{nobjects.NAobject}:
    methods for the generics \texttt{npoints} and \texttt{nobjects},
    which return an \texttt{NA} integer value.
\item \texttt{c.layered}: concatenate layered objects, retaining their plot
  arguments.
\item \texttt{identify.tess}: interactively identify tiles of a tessellation.
\item \texttt{identify.linnet}: interactively identify segments of a network.
\item \texttt{identify.lintess}: interactively identify tiles of a tessellation
  on a network.
\item \texttt{as.linfun.linnet}: create a function on a linear network which
  maps each network segment to a specified value.
...
\item \texttt{addvar}:
  \begin{itemize}
  \item Now accepts point process models of class 
    \texttt{ppm}, \texttt{kppm}, \texttt{dppm}, \texttt{rppm}, \texttt{lppm}.
  \item For non-Poisson models, significance bands are computed by simulation.
  \end{itemize}
...
\item \texttt{anyDuplicated.ppp}: 
  \begin{itemize}
  \item  New argument \texttt{rule} supports different levels of accuracy
    consistent with the options for \texttt{duplicated.ppp}.
  \item   Accelerated.
  \end{itemize}
...
\item \texttt{applynbd}:
 ...
 \item \texttt{as.function.tess}:
  ...
  \item \texttt{as.fv.kppm}:
    Improved arguments for empirical estimation of pair correlation function
    when the model was fitted with a non-default value of \texttt{method}.

  \item \texttt{as.im}: 
    \begin{itemize}
      \item Many methods for \texttt{as.im}
        now have argument \texttt{rule.pix} passed to \texttt{owin2mask}.
         ...
         by setting \texttt{rule.pix="cover"}.
    \item 
      Many methods for \texttt{as.im}
  ...
  \item \texttt{as.im.tess}:
    \begin{itemize}
    \item
      New argument \texttt{rule.tile} specifies how to determine,
...
\item \texttt{as.layered.ppp}:
  Now handles rejected points in \texttt{attr(x, "rejects")} and 
  retrieves them as a layer.
...
\item \texttt{as.linim.linfun}:
 ...
 \item \texttt{as.linnet.linnet}:
  ...
  \item \texttt{as.linnet.psp}:
    \begin{itemize}
    \item New arguments \texttt{chop} and \texttt{fuse}.
    \item 
...
%%B
 ...
 \item \texttt{clarkevans.test}:
  ...
  \item \texttt{clickpoly}:
    \begin{itemize}
    \item     The polygon is now drawn progressively as the user clicks new vertices.
    \item     Allows user-selected locations of vertices to be snapped (rounded)
    to the nearest grid points. New arguments \texttt{snap.step} and 
    \texttt{snap.origin}.
    \end{itemize}
 ...
 \item \texttt{clusterstrength}: 
   Now accepts an object of class \texttt{"clustermodel"}.
...
\item \texttt{connected.owin}:
  \begin{itemize}
  \item New argument \texttt{polygonal} allows calculation of 
    connected components using polygonal geometry instead of pixel-based
    algorithm.
  \item Arguments \texttt{...} now determine pixel resolution.
  \end{itemize}
  ...
  \item \texttt{cor.im}, \texttt{cov.im}:
    Default changed to \texttt{use="complete.obs"}.
...
%%D
 ...
 \item \texttt{default.symbolmap.ppp}:
   \begin{itemize}
   \item For a multitype point pattern, \texttt{cols} can be 
     a \texttt{function(n)} 
   \item New argument \texttt{scramble.cols}
   \item 
 ...
 \item \texttt{density.lpp}:
   \begin{itemize}
   \item 
     Argument \texttt{weights} can be logical valued.
   \item 
 ...
 \item \texttt{density.ppp}:
   \begin{itemize}
   \item Now accepts argument \texttt{rule.pix} passed to \texttt{owin2mask}.
   \item Argument \texttt{weights} can be logical-valued.
 ...
 \item \texttt{dfbetas.ppm}:
  ...
  \item 
   ...
   \item diagnostic functions:
     \begin{itemize}
     \item The following functions have new formal argument \texttt{rmax}:
       \begin{quote}
         \texttt{Gcom}, \texttt{Kcom}, \texttt{psstA}, \texttt{psstG}, \texttt{psst}
       \end{quote}
     \item The following functions accept argument \texttt{rmax}:
       \begin{quote}
         \texttt{Gres}, \texttt{Kres}
       \end{quote}
     \item The following functions accept \texttt{correction="all"}:
       \begin{quote}
         \texttt{Gcom}, \texttt{Gres}, \texttt{Kcom}, \texttt{Kres}
       \end{quote}

     \end{itemize}
  \item \texttt{diagnose.ppm}:
    \begin{itemize}
    \item This is now a method for the generic \texttt{diagnose}.
    \item 
 ...
 \item \texttt{disc}:
    New argument \texttt{type} allows the user to specify an inscribed polygon
 ...
 \item \texttt{distfun.owin}:
   New argument \texttt{signed}.
   
 \item \texttt{distmap.owin}:
   \begin{itemize}
   \item    New argument \texttt{signed}.
...
\item \texttt{distmap.ppp}:
  \begin{itemize}
  \item 
    New option \texttt{squared=TRUE} returns the squared distances,
    reducing computation time.
  \item 
    New option \texttt{extras=FALSE} avoids returning additional attributes,
    reducing computation time.
  \item    New argument \texttt{clip}.
  \end{itemize}
...
%%E
  ...
  \item \texttt{edge.Ripley}:
    \begin{itemize}
    \item New argument \texttt{bdistX} for efficiency.
    \item Responds more quickly to user interrupt signal.
    \end{itemize}
  ...
  \item \texttt{effectfun}:
    \begin{itemize}
    \item Improved defaults for a recursive partition model 
      (class \texttt{"rppm"}).
      ...
      variable, then \texttt{covname} is not needed.
    \item If a spatial covariate is constant, its value does not need to
      be provided in the call to \texttt{effectfun}.
    \item 
  ...
  \item \texttt{Emark}:
    The result now has an attribute \texttt{smooth.args}
 ...
 \item \texttt{envelope}:
   \begin{itemize}
   \item 
     Argument \texttt{simulate} can be a point process model of class
    \texttt{"ppm"}, \texttt{"kppm"}, \texttt{"dppm"}, \texttt{"slrm"}, 
    \texttt{"clusterprocess"} or \texttt{"detpointprocfamily"}.
 \item 
   New argument \texttt{theoryfun} can be used to provide the theoretical value
 ...
 \item \texttt{envelope.pp3}:
   \begin{itemize}
   \item 
   Marked point patterns are accepted.
 \item 
   New arguments \texttt{fix.n} and \texttt{fix.marks} determine whether the 
   simulated patterns have a fixed or random number of points.
   \end{itemize}

 \item \texttt{erosion.owin}:
   when \texttt{shrink.frame=TRUE} (the default), 
   the  frame of the resulting window
    is always the erosion of the original frame by distance \texttt{r},
...
%%F
 ...
 \item \texttt{fitted.slrm}:
   \begin{itemize}
   \item
     Argument \texttt{leaveoneout} is no longer ignored when 
     \texttt{dataonly=FALSE};
 ...
 \item \texttt{FmultiInhom}:  New argument \texttt{rmax}.
...
%%G
 ...
 \item \texttt{Gcross}:
   \begin{itemize}
   \item New argument \texttt{rmax}.
   \item    Function labels (shown on the plot legend) have been improved when 
   \texttt{i = j}.
...
%%H
   ...
   \item \texttt{harmonise.im}:
     \begin{itemize}
     \item Accelerated in the common case where all rasters are identical.
     \item  The result belongs to classes \texttt{solist} and \texttt{imlist}
 ...
 \item \texttt{hyperframe}: 
   ...
   to conform to changes in R. (The actual default value is \texttt{TRUE}
   for \texttt{R < 4.1.0} and \texttt{FALSE} for \texttt{R >= 4.1.0}).
 \item     Removed slight inconsistencies in the internal format
...
%%I
 \item \texttt{identify.ppp}: 
   ...
   \begin{itemize}
   \item New arguments \texttt{paint} and \texttt{paint.args}.
 ...
 \item \texttt{idw}: Standard errors can now be calculated by
  ...
  \item \texttt{im.apply}: 
    \begin{itemize}
    \item Now handles vector-valued functions.
    \item Computation accelerated
 ...
 \item \texttt{integral.linfun}:
   \begin{itemize}
   \item New argument \texttt{exact}. 
     If \texttt{exact=TRUE}, a more accurate algorithm is used.
   \item Argument \texttt{domain} can be a tessellation on a network
     (class \texttt{lintess}).
   \item New argument \texttt{weight} specifies a weight function for the integration.
...
\item \texttt{intersect.owin}:
  New argument \texttt{rescue}.
...
%%K
 ...
 \item \texttt{Kcross.inhom}, \texttt{Kdot.inhom}, \texttt{Kmulti.inhom}:
   \begin{itemize}
   \item 
     Now recognise argument \texttt{ratio}.
     If \texttt{ratio=TRUE}, the numerator and denominator of each estimate
     are saved, for use in analysing replicated point patterns.
   \item  
     Interpretation of arguments \texttt{lambdaI}, \texttt{lambdaJ}, 
     \texttt{lambdaX}, \texttt{lambdadot}
     ...
     entire point pattern \texttt{X}, 
     while if the model is an unmarked point process
...
\item \texttt{kernel.moment}:
 ...
 \item \texttt{kppm}:
   \begin{itemize}
   \item New option: \texttt{method="waag"} fits the model by maximising
...
\item \texttt{Ldot}:
 ...
 \item \texttt{lengths.psp}: 
   ...
   \item \texttt{levelset}:
     \begin{itemize}
     \item This function is now generic and can be applied to pixel images
       or functions. 
     \item For some kinds of functions, the level set is calculated
       more accurately using analytic geometry and returned as a polygonal
       window. This includes
       \begin{itemize}
       \item Functions of class \texttt{"distfun"} created bt \texttt{distfun}
         where the level set is a dilation of the original object
       \item Functions of class \texttt{tessfun} created by
         \texttt{as.function.tess} where the level set is a union of tiles
         of the tessellation.
       \end{itemize}
     \end{itemize}
 ...
 \item \texttt{linearKdot.inhom}, \texttt{linearpcfdot.inhom}:
   \begin{itemize}
   \item 
      When argument \texttt{lambdaI} or \texttt{lambdadot}
      is a point process model,
      ...
      relevant type of point (type \texttt{i} only, 
      or total intensity for all types of points).
   \item 
 ...
 \item \texttt{linearKcross.inhom}, \texttt{linearpcfcross.inhom}
   \begin{itemize}
   \item 
      When argument \texttt{lambdaI}, \texttt{lambdaJ} or \texttt{lambdadot}
      is a point process model,
      the interpretation now depends on whether the model is multitype or
      unmarked.  A multitype point process model will be interpreted as
      providing the intensity for each possible type of point. An unmarked point
      process model will be interpreted as providing the intensity only for the
      relevant type of point (type \texttt{i} only, type \texttt{j} only, 
      or total intensity for all types of points).
   \item 
...
\item \texttt{linearKinhom}, \texttt{linearpcfinhom}:
 ...
 \item \texttt{linequad}:
   Argument \texttt{Y} can be a linear network.
 ...
 \item \texttt{lppm}:
   \begin{itemize}
   \item The model formula may involve the local coordinates
     \texttt{seg} and \texttt{tp}.
...
\item \texttt{lurking.lppm}:
  \begin{itemize}
  \item Argument \texttt{covariate} can be a \texttt{function(x,y)}
    or one of the strings \texttt{"x"} or \texttt{"y"}.
  \item When the covariate is a cartesian coordinate or a distance function,
    the unit of length is stated on the horizontal axis label of the plot.
  \end{itemize}
  
\item \texttt{lurking.ppm}:
  \begin{itemize}
  \item Argument \texttt{covariate} can be a \texttt{function(x,y)}
    or one of the strings \texttt{"x"} or \texttt{"y"}.
  \item When the covariate is a cartesian coordinate or a distance function,
...
%%M
 ...
 \item \texttt{markconnect}:
   ...
   \item 
    ...
    containing the smoothing parameters used.
   \item 
     Accepts the argument \texttt{weights} 
     which is passed to \texttt{markcorr}.
   \end{itemize}
 ...
 \item \texttt{markcorr}:
  ...
  \item 
   ...
   \item 
     ...
     (NOTE: interpretation of \texttt{weights} has changed in 
     \texttt{spatstat.explore} version \texttt{3.6-1})
   \item 
...
\item \texttt{markcrosscorr}:
  \begin{itemize}
   \item Argument \texttt{weights} can be logical-valued.
  \item New argument \texttt{weights}.
  \item Now allows negative mark values, when \texttt{normalise=FALSE}.
  \end{itemize}
...
\item \texttt{marktable}:
 ...
 \item \texttt{mppm}:
   \begin{itemize}
   \item 
     The responses can be point patterns on a linear network
     (objects of class \texttt{"lpp"}).
   \item 
     Recognises argument \texttt{quad.args}.
   \item 
 ...
 \item \texttt{msr}:
  ...
  \item \texttt{multiplicity}:
    Methods for \texttt{multiplicity} can now handle objects of larger size.
...
%%N
 ...
 \item \texttt{nncross.default}:
   \begin{itemize}
   \item 
   New option \texttt{squared=TRUE}
   returns the squared nearest neighbour distances,
   reducing computation time.
 \end{itemize}
...
%   \item new argument \texttt{dmax}.
 ...
 \item \texttt{nndist.default}:
   New option \texttt{squared=TRUE}
   returns the squared nearest neighbour distances,
   reducing computation time.

 \item \texttt{nndist.ppp}:
   \begin{itemize}
   \item 
     New option \texttt{squared=TRUE}
     returns the squared nearest neighbour distances,
...
%%O
 ...
 \item \texttt{objsurf} methods:
   Arguments \texttt{xlim,ylim} renamed \texttt{par1lim,par2lim}.
   ...
   \item \texttt{owin2mask}:
     \begin{itemize}
     \item 
       Arguments \texttt{'...'} that are not recognised by \texttt{as.mask}
     ...
     \item 
        Changed formal arguments. First argument is now called \texttt{w}.
        Formal argument \texttt{op} renamed \texttt{rule.pix}.
...
%%P
 ...
 \item \texttt{palmdiagnose}:
   \begin{itemize}
   \item Now accepts objects of class \texttt{dppm} as well as \texttt{kppm}.
   \item New argument \texttt{delta} as an alternative to \texttt{breaks}.
   \end{itemize}
 \item \texttt{parres}: 
 ...
 \item \texttt{pcf.ppp}:
   \begin{itemize}
   \item The default value of \texttt{divisor} has temporarily reverted to 
     \texttt{"r"} pending further investigation.
   \item Changed rule for converting bandwidths when 
     \texttt{divisor="a"}.
   \item The default value of \texttt{divisor} has changed to \texttt{"a"},
    so that the new area-based estimator is the default.
  \item
    The default value of \texttt{zerocor} is now \texttt{"JonesFoster"}
    for small patterns and \texttt{"convolution"} for larger patterns.
  \item 
    New argument \texttt{nsmall} determines the threshold for large patterns.
 ...
 \item \texttt{pcfcross}, \texttt{pcfdot}:
   Shortened list of formal arguments (other arguments pass to
   \texttt{pcfmulti}).
 ...
 \item \texttt{pcfcross}, \texttt{pcfdot}, \texttt{pcfmulti}:
   ...
   \item The default value of \texttt{divisor} has temporarily reverted to 
     \texttt{"r"} pending further investigation.
   \item The default estimation method has changed. The new default is
    based on squared distances (\texttt{divisor="a"}).
   \item 
 ...
 \item \texttt{pcfcross.inhom}, \texttt{pcfdot.inhom}:
   \begin{itemize}
   \item 
     Now recognise argument \texttt{ratio}.
    If \texttt{ratio=TRUE}, the numerator and denominator of each estimate
 ...
 \item \texttt{pcfinhom}:
  ...
  \item 
    New argument \texttt{nsmall} determines the threshold for large patterns.
  \item Default value of \texttt{normpower} has changed to 2.
   \item New option: \texttt{divisor="a"} calculates a new estimator of the pcf
    with improved performance.
   \item  
     New arguments \texttt{adjust.sigma} and \texttt{adjust.bw} allow
 ...
 \item \texttt{pcfmulti}:
       New arguments \texttt{h}, \texttt{bw.args}, \texttt{adjust}, 
       \texttt{zerocor}, \texttt{nsmall}, \texttt{gref}, 
       \texttt{tau}, \texttt{close}.
 ...
 \item \texttt{persist}: Now accepts an object of class \texttt{"clustermodel"}.
 ...
 \item \code{persp.im}:
   \begin{itemize}
   \item The pixel values of \texttt{colin} can be colour values.
     New argument \texttt{valuesAreColours}.
   \item     Now recognises argument \texttt{adj.main}
     controlling the position of main title.
   \item     Improved appearance when \texttt{apron=TRUE}
     and the image domain is not a rectangle.
   \end{itemize}

 \item \code{persp.linim}:
   \begin{itemize}
   \item  Improved behaviour close to vertices of the network.
   \item  New arguments \texttt{extrapolate} and \texttt{zadjust}.
   \item Argument \texttt{col.base} can be a pixel image, allowing
     a colour image to be plotted on the horizontal plane at height zero.
   \end{itemize}

   
 \item \code{persp.ppp}:
   \begin{itemize}
   \item Now plots the \texttt{zlab} label alongside the vertical scale bar.
   \item Argument \texttt{col.base} can be a pixel image, which will be
     rendered as a colour image in perspective view on the horizontal plane.
   \item New argument \texttt{show.window}.
   \item 
     Now recognises argument \texttt{adj.main}
     controlling the position of main title.
   \end{itemize}
   
  \item \texttt{pixellate.owin}:
    \begin{itemize}
    \item Now accepts the argument \texttt{rule.pix} passed 
      to \texttt{owin2mask}.
      This makes it possible to ensure that boundary pixels which partially
      intersect the window are included in the resulting pixel image,
      by setting \texttt{rule.pix="cover"}.
 ...
 \item \texttt{plot.im}:
   \begin{itemize}
   \item New argument \texttt{opacity} controls semi-transparency of colours.
   \item The position of text next to the colour ribbon can now be
     controlled by \verb!riblab$side!. 
   \item Placement of main title improved in some cases.
 ...
 \item \texttt{plot.kppm}:
   \begin{itemize}
   \item 
     For models fitted by Palm likelihood or composite likelihood, the plot
     selected by \texttt{what="statistic"}
     displays the pair correlation function.
   \item 
 ...
 \item \texttt{plot.lintess}:
   ...
   \item \texttt{plot.owin:} 
     ...
     \item 
       ...
       in the main title.
     \item 
       New argument \texttt{background} specifies an object (or a colour)
       that will be plotted before \texttt{x} is plotted, and will
  ...
  \item \texttt{plot.pp3}:
   ...
   \item \texttt{plot.ppp}:
     \begin{itemize}
     \item New argument \texttt{show.rejects}.
     \item New argument \texttt{scramble.cols}
     \item New arguments \texttt{background} and \texttt{clip.background} allow
       the user to specify an object (or a colour value)
       that will be plotted before the point 
       pattern \texttt{x}, and will therefore appear underneath it.
 ...
 \item \texttt{ppp}:
  ...
  \item \texttt{predict.slrm}:    
    \begin{itemize}
    \item New argument \texttt{leaveoneout} 
       supports ``leave-one-pixel-out'' prediction.
    \item New argument \texttt{fast} 
      determines algorithm used when \texttt{leaveoneout=TRUE}.
    \end{itemize}
  ...
  \item \texttt{print.lintess}:
    \begin{itemize}
    \item Output improved.
    \item Output includes information about marks.
    \end{itemize}
  ...
  \item \texttt{print.slrm}: Prints information about the pixel raster.
    
  \item \texttt{print.summary.owin}:
    Output can now be abbreviated by
    increasing the value of \texttt{spatstat.options('terse')}
    
  \item \texttt{print.summary.psp}:
    Output can now be abbreviated by
    increasing the value of \texttt{spatstat.options('terse')}
  ...
  \item \texttt{projectsegment}: Accelerated by 20 percent.

  \item \texttt{projectset}: Supports projection to a polygonal window;
    new argument \texttt{polygonal}.
  ...
  \item \texttt{psib}: Now accepts an object of class \texttt{"clustermodel"}.
...
%%R
  ...
  \item \texttt{rbind.hyperframe}:
   ...
   \item \texttt{residualMeasure}:
     More options for argument \texttt{lambda}.
   ...
   \texttt{rhohat}:
   \begin{itemize}
   \item  Methods for \texttt{rhohat} now recognise the argument \texttt{delta}
    as an alternative to argument \texttt{breaks} when 
    \texttt{smoother="piecewise"}.
 ...
 \item \texttt{rjitter.ppp}:  If \texttt{trim=TRUE}, 
  ...
  \item \texttt{rmhmodel.ppm}, \texttt{simulate.ppm}:
    New argument \texttt{newdata} provides support for simulating
    a fitted model in a different window.
 ...
 \item \texttt{rmh.default, rmhmodel.default}:
  ...
  \item \texttt{rmpoint}:
          New argument \texttt{fail.action} specifies what to do
      if the required number of random points cannot be generated. 
...
\item \texttt{roc.ppm}, \texttt{roc.kppm}, \texttt{roc.slrm}:
  \begin{itemize}
  \item New argument \texttt{baseline} allows calculation of ROC
    relative to a baseline.
  \item New argument \texttt{method} determines the estimation method.
    New options include a kernel smoothing estimate and a monotone estimate.
  \item New arguments \texttt{CI}, \texttt{alpha} for calculating 
    confidence intervals.
  \item New argument \texttt{subset} specifies a sub-region of the
    spatial domain in which the ROC should be calculated.
  \item New argument \texttt{leaveoneout} specifies whether to use
    leave-one-out estimates of intensity at the data points.
  \end{itemize}
  ...
  \item New argument \texttt{baseline} allows calculation of ROC
    relative to a baseline.
  \item New argument \texttt{weights} for numerical weights on the data points.
  \item New argument \texttt{observations} specifies whether to use the
    exact point coordinates or pixel presence-absence indicators.
  \item New argument \texttt{method} determines the estimation method.
    New options include a kernel smoothing estimate and a monotone estimate.
  \item New arguments \texttt{CI}, \texttt{alpha} for calculating 
    confidence intervals.
...
\item \texttt{roc.slrm}:
  \begin{itemize}
  \item  Changed interpretation of argument \texttt{leaveoneout=TRUE}.
  ...
  \end{itemize}
   ...
   \item \texttt{rose}
     \begin{itemize}
     \item Rose diagrams can now be computed and plotted for data
       expressed in any units with any period (such as hours in a 24-hour day).
     \item
       More options for argument \texttt{unit}.
     \item
       New argument \texttt{fullcircle}.
     \item 
       New arguments \texttt{start} and \texttt{clockwise} 
 ...
 \item \texttt{rose.default}
   \begin{itemize}
   \item Polygons are now filled in light grey (by default)
   \item New argument \texttt{col} specifies fill colour
   \item New argument \texttt{do.ticks} controls whether ticks are drawn
   \item Concentric rings are now drawn inside the circle, for scale.
   \item New argument \texttt{do.rings}.
   \item The return value now has attribute \texttt{R} giving the radius
     of the outer circle of the diagram, and attribute \texttt{rings} 
     giving the radii of the concentric rings.
   \item New argument \texttt{root} specifies whether to apply a 
     square root transformation.
   \item    New argument \texttt{weights}.
   \end{itemize}

 \item \texttt{rose.density}
   \begin{itemize}
  \item New argument \texttt{do.ticks} controls whether ticks are drawn
   \item Concentric rings are now drawn inside the circle, for scale.
   \item New argument \texttt{do.rings}.
 ...
 \item \texttt{rose.fv}
  ...
  \item New argument \texttt{do.ticks} controls whether ticks are drawn
   ...
   \item The return value now has attribute \texttt{R} giving the radius
     of the outer circle of the diagram, and attribute \texttt{rings} 
     giving the radii of the concentric rings.
   \item Now plots multiple curves if there are multiple columns of values.
   \item New argument \texttt{fmla} controls which columns are plotted.
   \item Now handles envelopes (objects of class \texttt{"envelope"}) as well.
   \item New argument \texttt{root} specifies whether to apply a 
     square root transformation.
   \end{itemize}
 
 \item \texttt{rose.histogram}
     \begin{itemize}
   \item Polygons are now filled in light grey (by default)
   \item New argument \texttt{col} specifies fill colour
  \item New argument \texttt{do.ticks} controls whether ticks are drawn
 ...
 \item \texttt{rotmean}:
   ...
   \item \texttt{rpoispp3}:
     \begin{itemize}
     \item New argument \texttt{ex} (example point pattern, to be emulated)
     \item Argument \texttt{lambda} may now be a \texttt{function(x,y,z)}
       so that an
       inhomogeneous Poisson process may be simulated.
     \item New argument \texttt{lmax}.
     \end{itemize}
   ...
   \item \texttt{rSSI}: 
     \begin{itemize}
     \item Now supports three-dimensional random point patterns.
    Argument \texttt{win} can be a window (class \texttt{owin})
    or a three-dimensional box (class \texttt{box3}).
     \item Accelerated.
 ...
 \item \texttt{rStrauss, rHardcore, rStraussHard, rDiggleGratton, rDGS, rPenttinen:}
   ...
   \item \texttt{runifpoint3}:
     New argument \texttt{ex} (example point pattern, to be emulated)
...
%%S   
  ...
  \item \texttt{setminus.owin}: new argument \texttt{rescue}.
  ...
  \item Simulation:
   ...
   \item \texttt{simulate.kppm}:
    ...
    the function that performs the simulation.
     ...
     \begin{itemize}
     \item  New argument \texttt{newdata} provides support for simulating
    a fitted model in a different window.
     \item 
   ...
   \item \texttt{slrm}:
     \begin{itemize}
     \item New argument \texttt{ruleAtPoints}
       controls discretisation rule at data points.
     \item Arguments \texttt{"..."} are now passed to 
       \texttt{owin2mask} to determine the
       polygon-to-raster discretisation rule for a covariate of class 
       \texttt{owin}.
 ...
 \item \texttt{Smooth.ppp}:
   \begin{itemize}
   \item Now accepts argument \texttt{rule.pix} passed to \texttt{owin2mask}.
   \item Shrinkage estimate implemented. New arguments
     \texttt{shrink} and \texttt{shrinktype}.
   \item Slight change to algorithms for handling very small bandwidths.
   \item A non-Gaussian kernel can now be specified
   using the argument \texttt{kernel}.
 ...
 \item \texttt{solapply}:  If an entry of the list is an 
   \texttt{NAobject}, the result for that entry will be an \texttt{NAobject}.
 ...
 \item \texttt{spatialcdf}: 
  ...
  \item summary functions:
    The following functions have a new argument \texttt{rmax}:
    \begin{quote}
        \texttt{Fest}, \texttt{Finhom}, \texttt{FmultiInhom}, 
        \texttt{Gcross}, \texttt{Gdot}, \texttt{Ginhom},
        \texttt{Gmulti}, \texttt{GmultiInhom},
        \texttt{Jest}, \texttt{Jcross}, \texttt{Jdot}, \texttt{Jinhom}, 
        \texttt{Jmulti},
        \texttt{Kcross}, \texttt{Kdot}, \texttt{Kinhom}, \texttt{Kmulti}, 
	\texttt{Kcross.inhom}, \texttt{Kdot.inhom}, \texttt{Kmulti.inhom}, 
        \texttt{Kmark}, \texttt{Ksector}, 
        \texttt{markconnect}, \texttt{markcorr}, 
        \texttt{markcorrint}, \texttt{markcrosscorr}, \texttt{markequal},
        \texttt{markvario}, \texttt{pcf.ppp}, \texttt{pcfcross}, 
        \texttt{pcfdot}, \texttt{pcfmulti}, \texttt{pcfinhom}
        \texttt{pcfcross.inhom}, \texttt{pcfcross}.
    \end{quote}
...
%%T
  ...
  \item \texttt{texturemap}:
   ...
   \item \texttt{thinNetwork}:
    ...
    \item \texttt{tileindex}:
      New arguments \texttt{close.gaps} and \texttt{all.inside}.
...
%%U
  ...
  \item \texttt{union.owin}:
    \begin{itemize}
    \item New argument \texttt{rescue}.
    \item Improved behaviour when there are more than 2 windows.
    \end{itemize}
 ...
 \item \texttt{uniquemap}:
   ...
   \item \texttt{unnormdensity}:
     \begin{itemize}
     \item Argument \texttt{bw} may be \texttt{NULL}
       (treated as equivalent to being missing).
     \item  The result has an attribute \texttt{smooth.args}
...
%%W
   ...
   \item \texttt{with.hyperframe}: 
     If a column in the hyperframe contains \texttt{NAobject} entries,
    and if the name of that column is included in the expression
    being evaluated, then the result of evaluation is an \texttt{NA} or 
    \texttt{NAobject} in the relevant row.
%%X
%%Y
%%Z
 ...
 \item \verb![<-.hyperframe!:
   \begin{itemize}
   \item In an assignment like \verb!x[i,j] <- NA!, if column \texttt{j}
     contains spatial objects, the \texttt{NA} value will be coerced to an
     \texttt{NAobject} of the appropriate class.
   \item Improved error message when the format of the index is not supported.
   \end{itemize}
 ...
 \item \verb![.im!
   New argument \texttt{rule.pix} controls the handling of boundary pixels
   in computations like \texttt{x[W]} where \texttt{W} is a polygonal window.
