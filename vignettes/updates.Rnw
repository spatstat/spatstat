...
% $Revision: 1.53 $ $Date: 2021/11/21 02:03:40 $
...
\begin{itemize}
  ...
  \item New package \pkg{spatstat.random}.
    
  \item the original \spst\ package has now been split into 8 sub-packages
    (\pkg{spatstat.utils}, \pkg{spatstat.data}, \pkg{spatstat.sparse},
    \pkg{spatstat.geom}, \pkg{spatstat.random},
    \pkg{spatstat.core}, \pkg{spatstat.linnet}
    and \pkg{spatstat}).
...
\item \texttt{zgibbsmodel}:
  Experimental. An object of class \texttt{zgibbsmodel} represents a
    ...
    (whereas \texttt{ppm} represents a model fitted to data).
\end{itemize}
