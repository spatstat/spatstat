...
\Spst\ has tripled in size since 2010,
...
\begin{itemize}
 ...
 \item \spst\ now `Depends' on the library \pkg{nlme}.
 ...
 \item geometrical errors in polygon data are now repaired automatically.
  ...
  \item Improvements to Gibbs models (\texttt{ppm}):
   ...
   \item New argument \texttt{subset} makes it possible to fit the
    ...
    \item 
     new 'logistic likelihood' fitting method;
   \item
     new variational Bayes fitting method;
   \item 
   ...
   \item 
     hierarchical interactions between types
     in a multitype point process.
    \end{itemize}
\item 
...
\item 
   ...
   \item
     Left hand side of formula can now involve entries in the 
     list \texttt{data}.
   \item 
     confidence intervals using \texttt{confint}
   \item
     \texttt{AIC} and \texttt{step} now work for \texttt{"kppm"} objects
     fitted using Palm likelihood.
   \item 
     covariates can be tessellations.
   \item 
     more control over the optimisation algorithm.
    \end{itemize}
\item
...
\item
    ...
    \item 
      tools for directional data (rose diagram, kernel smoothing)
    \end{itemize}
\item
...
\item
    simulation:
    \begin{itemize}
    \item Basic simulation algorithms accelerated.
    \item 
...
\item mathematical operations are now supported for images.
...
\item vignettes:
   ...
   'Datasets in the Spatstat Package'
...
\item \texttt{anylist}, \texttt{solist}:
  these are replacements for the class '\texttt{listof}'.
  An object of class '\texttt{anylist}' represents a list of
  objects, of any kind, that the user intends to treat in the same way.
  An object of class '\texttt{solist}' (spatial object list)
  represents a list of two-dimensional spatial objects.
\end{itemize}
...
\item \texttt{dendrite}:
  Dendritic spines on a branch of the dendritic tree of a neuron.
...
\item \texttt{vesicles}:
  Spatial pattern of synaptic vesicles in rat brain tissue.
  A point pattern with covariates.
...
\item \texttt{dppBessel}, \texttt{dppCauchy}, 
 ...
 \item    
   \texttt{update.dppmodel}: Set parameter values in a \texttt{dpp}.
 \item \texttt{is.stationary.dppmodel}, 
   \texttt{print.dppmodel}, \texttt{reach.dppmodel}, \texttt{valid.dppmodel}: 
   Basic information about a \texttt{dpp} model.
 \item
   \texttt{rdpp}, \texttt{simulate.dppmodel}:
   Simulation of a \texttt{dpp} model.
 \item 
   \texttt{intensity.dppmodel}, \texttt{Kmodel.dppmodel}, 
   \texttt{pcfmodel.dppmodel}: Moments of a \texttt{dpp} model.

 \item \texttt{dim.dppmodel}, \texttt{dppapproxkernel}, 
   \texttt{dppapproxpcf}, \texttt{dppeigen}, \texttt{dppfamily},
   \texttt{dppkernel}, \texttt{dppparbounds}, \texttt{dppspecdenrange}, 
   \texttt{dppspecden}:
   Helper functions for \texttt{dpp} models.
 ...
 \item \texttt{dclf.sigtrace}, \texttt{mad.sigtrace}, \texttt{mctest.sigtrace}:
 ...
 \item \texttt{fixef.mppm}, \texttt{ranef.mppm}:
 ...
 \item \texttt{to.opaque}, \texttt{to.transparent}:
 ...
 \item \texttt{PPversion}, \texttt{QQversion}:
 ...
 \item \texttt{valid}:
   ...
   \texttt{valid.ppm}, \texttt{valid.lppm}, \texttt{valid.dppmodel}.
...
\item \texttt{fitted.lppm}: fitted intensity values for a point process
  ...
  \item \texttt{HierStrauss}, \texttt{HierStraussHard}, \texttt{HierHard}:
    hierarchical interactions for Gibbs models.
    
  \item \texttt{density.lpp}:
    Kernel estimation of intensity on a linear network.
    
   \item \texttt{extractbranch}, \texttt{deletebranch}, 
     \texttt{treeprune}, \texttt{treebranchlabels}, \texttt{begins}:
     Tools for analysing data on a tree.
  
   \item \texttt{studpermu.test}:
   Studentised permutation test for difference between groups of point patterns.
   Generously contributed by Ute Hahn.

   \item \texttt{AIC.kppm}, \texttt{extractAIC.kppm}, 
     \texttt{logLik.kppm}, \texttt{nobs.kppm}:
   ...
   \item \texttt{edit.ppp}, \texttt{edit.psp}, \texttt{edit.im}:
   Interactive text editor works for spatial datasets.

   \item \texttt{edit.hyperframe}:
   ...
   \item \texttt{summary.kppm}, \texttt{print.summary.kppm}, 
     \texttt{coef.summary.kppm}:
   Methods for \texttt{summary} and \texttt{coef(summary(..))} 
   for Cox and cluster models.

   \item \texttt{as.im.funxy}:
   Method for \texttt{as.im} for class \texttt{"funxy"}.

   \item \texttt{shift.linim}, \texttt{scalardilate.linim}, 
     \texttt{affine.linim}:
   Geometrical transformations for \texttt{"linim"} objects.

   \item \texttt{Smooth.solist}:
   Smooth method for a list of spatial objects.

   \item \texttt{unitname.tess}, \texttt{unitname<-.tess}:
   Tessellations now keep track of the name of the unit of length.

   \item \texttt{dirichletAreas}:
   Faster algorithm for \texttt{tile.areas(dirichlet(X))}.

   \item \texttt{identify.lpp}
   Method for \texttt{identify} for point patterns on a linear network.

   \item \texttt{dirichletWeights}:
      This function will replace \texttt{dirichlet.weights}.
      The latter is now 'deprecated'.

 \item \texttt{clusterfield}, \texttt{clusterkernel}:
  Compute the cluster kernel of a cluster model,
   or compute the cluster field generated by superimposing copies 
   of the cluster kernel at specified locations.
 \item \texttt{clusterradius}: 
   compute the radius of clusters in a cluster model.
\item \texttt{integral}: new generic function with methods
  for classes \texttt{"im"}, \texttt{"msr"}, \texttt{"linim"}.
\item \texttt{Math.linim}: arithmetic for pixel images on a linear network.
  \item \texttt{iplot.linnet}, \texttt{iplot.lpp}: interactive plotting 
    for linear networks.
  \item \texttt{pixellate.linnet}: discretise a linear network
  ...
  \item \texttt{as.linnet.linim}:
    Extract linear network from a \texttt{"linim"} object.
  \item \texttt{selfcut.psp}:
   Cut line segments where they cross each other.
  \item \texttt{vertices.linnet}, \texttt{vertexdegree}:
    Extract the vertices (nodes) of the linear network
    and compute the degree of each vertex.
  \item \texttt{dirichletNetwork}, \texttt{delaunayNetwork}:
   The linear networks formed by the Dirichlet tessellation
   and Delaunay triangulation.
  \item \texttt{dirichletEdges}: the edges of the Dirichlet tessellation.
\item \texttt{quantess}:
   Quantile tessellation: divide space into pieces which contain
   equal amounts of 'stuff'.
 \item \texttt{nestsplit}: Nested split
 \item \texttt{marks.tess}, \verb!marks<-.tess!, \texttt{unmark.tess}:
   Extract or change the marks associated with the tiles of a tessellation.
 \item \texttt{clusterfield}:
   Compute the cluster kernel of a (fitted or theoretical) cluster model,
   or compute the cluster field generated by superimposing copies 
   of the cluster kernel at specified locations.
 \item \texttt{pool.fv}, \texttt{pool.anylist}
   New methods for \texttt{pool}
 \item \texttt{head.hyperframe}, \texttt{tail.hyperframe}:
     Methods for \texttt{head} and \texttt{tail} for hyperframes.
   \item \texttt{subset.hyperframe}:
     Method for \texttt{subset} for hyperframes.
   \item \texttt{as.function.im}:
     Converts a pixel image to a function of spatial coordinates.
   \item \texttt{clickdist}:
   Measures the distance between two spatial locations clicked by the user.
   \item \texttt{solapply}, \texttt{anylapply}:
   wrappers for \texttt{lapply} which return a list of class 
   \texttt{"solist"} or \texttt{"anylist"}.
   \item \texttt{Kmark}: Weighted $K$-function. 
   Identical to \texttt{markcorrint} and will eventually replace it.
  \item \texttt{segregation.test}
   Test of spatial segregation of types in a multitype point pattern.
  \item mathematical operations are now supported for images. For example\\ 
    \verb!alpha <- atan(bei.extra$grad) * 180/pi!   %$
  \item \texttt{relrisk.ppm}: spatially-varying probabilities
    of different types of points
    predicted by a fitted point process model.
  \item \texttt{circdensity}: Kernel density estimate for angular data
  \item \texttt{rose}: Rose diagram (rose of directions) for angular data
  \item \texttt{nnorient}: Nearest neighbour orientation distribution.
  \item \texttt{AIC.ppm}: Calculate \textit{AIC}
    of a Gibbs model using Takeuchi's rule.
  \item \texttt{interp.colours}:
   Interpolate a sequence of colour values.
  \item \texttt{anyDuplicated.ppp}, \texttt{anyDuplicated.ppx}:
   Fast replacements for \texttt{any(duplicated(x))} for point patterns.
  \item \texttt{textstring}, \texttt{onearrow}, \texttt{yardstick}:
   Objects representing a text string, an arrow, or a scale bar,
   for use in drawing spatial diagrams.
  \item \texttt{plot.imlist}, \texttt{image.imlist}, \texttt{contour.imlist}:
   Methods for the new class \texttt{"imlist"}
  \item \verb![<-.layered!,  \verb![[<-.layered!:
   More support for class \texttt{"layered"}
  \item \texttt{anylist}, \texttt{as.anylist}, \texttt{[.anylist}, \texttt{[<-.anylist}, \texttt{print.anylist}, \texttt{summary.anylist}\\ 
   Support for new class '\texttt{anylist}'
 \item \texttt{solist}, \texttt{as.solist}, 
   \texttt{[.solist}, \texttt{[<-.solist}, \texttt{print.solist}, 
   \texttt{summary.solist}\\
   Support for new class '\texttt{solist}'
 \item \texttt{plot.anylist}, \texttt{plot.solist}: \\
   Plot methods for the classes '\texttt{anylist}' and '\texttt{solist}'
   (Currently identical to '\texttt{plot.listof}')
\item \texttt{perspPoints}, \texttt{perspLines}, \texttt{perspSegments},
  \texttt{perspContour}: draw points and lines on a surface,
  as seen in perspective view.
\item \texttt{project2set}: 
  For each data point in a point pattern \texttt{X}, find the nearest
  spatial location in a given window \texttt{W}.
\item \texttt{discs}: union of discs. 
  (Given a point pattern dataset recording the locations and diameters
  of objects, find the region covered by the objects.)
\item \texttt{Fhazard}: hazard rate of the empty space function.
\item \texttt{dirichletVertices}: Vertices of the Dirichlet tessellation.
\item \texttt{stienen, stienenSet}: Stienen diagrams.
\item \texttt{hopskel.test}: Hopkins-Skellam test.
\item \texttt{is.grey}: determines whether a colour value is a shade of grey.
...
\item \code{textureplot}: 
  Display a tessellation, or a factor-valued pixel image, using texture fill.
...
\begin{itemize}
   ...
   \item \spst\ now depends on \texttt{R 3.2.2} or later.
   ...
   \item Bug fix in Metropolis-Hastings simulation.
   ...
   \item New dataset: Australian states and territories.

   \item More support for multi-dimensional point patterns.
 ...
 \item Improvements to simulation of Neyman-Scott processes.
   ...
   \item Permutation test for difference between groups of point patterns.

   \item Variational Bayes estimation for point process models.

   \item Score test in \texttt{anova.ppm}

   \item ROC curve, and discrimination index AUC, for fitted models.
   
   \item Interactive text editor for spatial datasets.

   \item Tools for analysing data on a tree. 

   \item Kernel density/intensity estimation on a linear network.
     
   \item Random pixel noise.

   \item Improved behaviour of polygon geometry operations.

   \item Improved support for cluster and Cox models.

   \item Improved basic support for owin objects.

   \item Improved support for tessellations.

   \item More hierarchical interactions for Gibbs models.
     
   \item Modifications to \texttt{Kest}.

   \item summary method for Cox and cluster models.

   \item class \texttt{"listof"} is almost completely replaced by 
     \texttt{"anylist"} and \texttt{"solist"}.

   \item Improvements and bug fixes.

 \item Reorganised parametrisation of cluster models.

  \item Sparse matrix representation of linear networks.
    
  \item More support for data on linear networks.
  
  \item Tessellations may now have marks.

  \item Nested splitting.

   \item Standard errors for kernel estimates of intensity.

   \item  Test for segregation.

   \item \texttt{spatstat} now imports (rather than Suggests)
     the package \texttt{Matrix}.
 
   \item \texttt{spatstat} no longer uses Fortran.
 
   \item \texttt{spatstat} no longer depends on 
     the package \texttt{scatterplot3d}.
 
   \item  Mathematical operators now apply to images.

   \item  Parametric estimates of relative risk 
     from fitted point process models.

   \item  Standard errors for relative risk (parametric and non-parametric).

   \item  Kernel smoothing and rose diagrams for angular data.

   \item  Perceptually uniform colour maps. 

   \item  Hierarchical interactions for multitype patterns.

   \item  Hard core parameters in all interactions no longer need
     to be specified and will be estimated from data.

   \item  Improvements to analysis of deviance and model selection.

   \item  New datasets.

   \item  New vignette, summarising all datasets installed with spatstat.

   \item  Tests and diagnostics now include a Monte Carlo option.

   \item  Faster checking of large datasets.

   \item  Faster simulations.

   \item  Code for drawing diagrams (arrows, scale bars).
 ...
 \item Likelihood cross-validation for smoothing bandwidth.
  ...
  \item Spatstat now has version nicknames.
   ...
   \item Palm likelihood method for fitting cluster processes and Cox processes.

   \item Quasi-likelihood and weighted composite likelihood methods
     for estimating trend in cluster processes and Cox processes.

   \item Further extensions to model formulas in ppm and kppm.

   \item Faster variance calculations for ppm objects.

   \item One-sided tests and one-sided envelopes of summary functions.

   \item Cramer-Von Mises and Anderson-Darling tests of spatial distribution.

   \item Cressie-Read test statistic in quadrat counting tests.

   \item Spatial cumulative distribution functions.

   \item Faster algorithm for point pattern matching.

   \item Increased support for envelopes.

   \item New generic functions \texttt{Window}, 
     \texttt{Frame} and \texttt{domain}.

   \item Directional $K$-function and directional distribution.

   \item Raster calculations accelerated.

   \item Summary functions accelerated.
     
   \item \texttt{anova} extended to Gibbs models.

   \item Improved algorithm for locally-scaled K-function.

   \item Leave-one-out calculation of fitted values in ppm objects

   \item Geometrical operations with windows and images.

   \item More edge corrections for K-function.

   \item Improved handling and plotting of 'fv' objects.

   \item Utilities for perspective views of surfaces.
...
listed alphabetically.
   ...
   \begin{itemize}
   \item \code{adaptive.density}:  
   Now accepts \texttt{f = 0} (uniform intensity estimate) and
   \texttt{f = 1} (Voronoi intensity estimatef) as well as \texttt{0 < f < 1}.
 ...
 \item \texttt{as.function.fv}:
  ...
  \item \texttt{alltypes}:
   Now works for \texttt{"lpp"} objects.
   New argument \texttt{reuse} determines whether all simulation envelopes
   are based on the same set of simulated patterns, or on independent sets.

  \item \texttt{allstats}:
   Result is now of class \texttt{"anylist"}.
   New argument \texttt{reuse} determines whether all simulation envelopes
   are based on the same set of simulated patterns, or on independent sets.

   \item \texttt{anova.lppm}: outdated argument \texttt{override}
     has been removed.
     
   \item \texttt{anova.ppm}: 
     Now performs the Score Test, for Poisson models only,
     if argument \texttt{test="Rao"} or \texttt{test="score"}.
 ...
 \item \texttt{ants}:
  ...
  \item \texttt{as.owin}, \texttt{as.mask}:
   \texttt{as.owin(D)} or \texttt{as.mask(D)} or \texttt{owin(mask=D)}
   will produce a binary mask window
   if \texttt{D} is a data frame with two columns of \texttt{(x,y)} coordinates
   or a data frame with three columns containing \texttt{(x,y,logical)}.

  \item {as.owin.data.frame}:
   \texttt{W} can now be a data frame with only two columns,
 ...
 \item \texttt{betacells}:
  ...
  \item \texttt{bw.diggle}:
    New argument \texttt{nr} controls accuracy.
 ...
 \item \texttt{by.ppp}:
   ...
   \item \texttt{clickpoly}, \texttt{clickbox}:
   These functions now handle graphical arguments to \texttt{polygon()}
 ...
 \item \texttt{collapse.fv}:
   Is now a method for the \pkg{nlme} generic \texttt{collapse}.
   Syntax adjusted slightly.
 ...
 \item \texttt{contour.im}: new argument \texttt{col} can be a colour map
   (to specify different colours for each contour level).

  \item \texttt{countends}:
    New argument \texttt{toler}.
 ...
 \item \texttt{crossdist.psp}: 
   The option \texttt{method="Fortran"} is no longer supported.
   The default is now \texttt{method="C"}.
 ...
 \item \texttt{crossing.psp}:
  ...
  \item \texttt{density.ppp}:
    \begin{itemize}
    \item    New argument \texttt{se} allows calculation of standard errors.
   ...
   \item New argument \texttt{positive} allows the user to stipulate that
   ...
   \item \texttt{diagnose.ppm}:
     ...
     \item 
       New arguments \texttt{col.neg}, \texttt{col.smooth} control the
       colour maps.
     \end{itemize}
   ...
   New argument \texttt{regularise} determines whether values at short distances
  ...
  \item \texttt{effectfun}:
   ...
   Now works for \texttt{"kppm"} and \texttt{"lppm"} objects as well.
   Argument \texttt{covname} is not needed if the model has only one covariate.

   \item \texttt{Emark}: 
     Now works with multiple columns.
     
   \item \texttt{envelope}: 
     \begin{itemize}
     \item One-sided envelopes can be selected
       by new argument \texttt{alternative}.
     \item 
     ...
     \item
       envelope methods now have argument \texttt{use.theory} specifying whether
       to use the ``theoretical'' value of the summary function
       when constructing simultaneous envelopes.
     \item 
       envelope methods have new argument \texttt{scale} specifying a 
       scaling function for global envelopes. The resulting global envelopes
       will have non-constant width, proportional to \code{scale(r)},
       instead of constant width.
     \item \texttt{envelope.envelope}:
       New argument \texttt{transform} allows the user to apply a transformation
       to previously-computed summary functions.
     \item \texttt{envelope.envelope}:
       In \texttt{envelope(E, fun=NULL)} 
       if \texttt{E} does not contain simulated
  ...
  \item \texttt{erosion}, \texttt{dilation}, \texttt{opening}, \texttt{closing}:
   ...
   New argument \emph{equiv} can be used to declare that two columns
 ...
 \item \texttt{identify.ppp}: now handles multivariate marks.
  ...
  \item \texttt{integral.im}, \texttt{integral.msr}:
   Argument \texttt{domain} can now be a tessellation;
 ...
 \item \texttt{intensity.ppm}:
  ...
  \item \texttt{intersect.owin}, \texttt{union.owin}, \texttt{setminus.owin}:
   Formal arguments \texttt{A} and \texttt{B} have been removed.
   Arguments may be lists of windows, of class \texttt{"solist"}.
   If inputs are polygons, the result is a polygon.
   New argument \texttt{p} controls resolution of polygon clipping algorithm.

 \item \texttt{intersect.tess}:
   Result now has the same 'unitname' as the input. 
  ...
  \item
    New argument \texttt{rmax} controls maximum value of argument \texttt{r}.
  \item 
 ...
 \item \texttt{Kcross}, \texttt{Kdot}, \texttt{Lcross}, \texttt{Ldot}:
   New argument \texttt{from} is an alternative for \texttt{i},
   and \texttt{to} is an alternative for \texttt{j}.
 ...
 \item 
  ...
  \item \texttt{AIC} and \texttt{step} now work for 
    \texttt{"kppm"} objects fitted by maximum Palm likelihood.
     \end{itemize}
 ...
 \item \texttt{linnet}: linear networks can be built using a
   sparse matrix representation to save memory.
...
\item \texttt{lohboot}:
  ...
  \item \texttt{lurking}:   
    Now returns an object of class \texttt{"lurk"} which has a plot method.
    Two-standard-deviation limits are now plotted using grey shading.

  \item \texttt{markcorr}:
    \begin{itemize}
    \item   Result is now of class \texttt{"anylist"}.
    \item New argument \texttt{weights}
      allows computation of the weighted version
  ...
  \item \texttt{marktable}:
   New argument \texttt{N} for studying the N nearest neighbours.   
   New argument \texttt{collapse} for manipulating the contingency table.
  ...
  \item \texttt{mppm}:
    \begin{itemize}
    \item Now handles models with a random effect component.
    ...
    \item Zero values in the interaction radii are now treated as \texttt{NA}.
    \item Improved handling of missing arguments.
    \item Printed output now respects options('width').
     \end{itemize}
 ...
 \item \texttt{nbfires}:
  ...
  \texttt{nbfires.extra}.
   ...
   New argument \texttt{k} specifies $k$-th nearest neighbour.
 ...
 \item \texttt{owin}: 
  ...
  \item \texttt{owin}:
   \texttt{owin(mask=D)} or \texttt{as.owin(D)} or \texttt{as.mask(D)}
   will produce a binary mask window
   if \texttt{D} is a data frame with two columns of \texttt{(x,y)} coordinates
   or a data frame with three columns containing \texttt{(x,y,logical)}.
  ...
  \item \texttt{pcfinhom}:
   ...
   \item \texttt{persp.im}:
	...
	The \texttt{colmap} argument can now be a \texttt{colourmap} object.
      ...
      \item 
        Option to compute which pixels are visible in the perspective view
        and which are obscured.
      \item Now obeys \texttt{spatstat.options("unitname")}.
      \end{itemize}
...
\item \texttt{plot.hyperframe}: 
 ...
 \item 
  ...
  \item \texttt{plot.cdftest}:
   ...
   \item \texttt{plot.imlist}:
    New argument \texttt{plotcommand} specifies how to plot each image.
 ...
 \item 
   New option: \texttt{method = "VBlogi"} for variational Bayes method.
 \item 
 ...
 \item Improved calculation of Dirichlet weights.
 \item 
   Confidence intervals for the fitted trend parameters
   can now be obtained using \texttt{confint} .
 \item 
...
\item \texttt{ppp}: 
  ...
  \item \texttt{print.mppm}:
    Now respects \verb!options('width')! and 
    \verb!spatstat.options('terse')!.
 ...
 \item \texttt{psst}:
  ...
  \item \texttt{quadratcount}:
  Argument \texttt{tess} can now be anything acceptable to as.tess.
    
  \item \texttt{quadrat.test.splitppp}:
   The result is now a single object of class \texttt{quadrattest}
   
  \item \texttt{quadrat.test.ppp}, \texttt{quadrat.test.quadratcount}:
   New argument \texttt{lambda} supports a test of the Poisson process 
   with given intensity \texttt{lambda}.
   ...
   \item \texttt{quadrats}:
     Result has the same unitname as the input.
     

   \item \texttt{quantile.ewcdf}:
     New argument \texttt{type} controls the type of quantile.
  ...
  \item \texttt{relrisk}:
    ...
    \item 
       ...
       point patterns and point process models.
     \item 
       New argument \texttt{casecontrol} determines whether a bivariate
       point pattern should be treated as case-control data.
     \item 
       New arguments \texttt{case} and \texttt{control} allow
       the user to specify which mark values
       corresponds to the cases or the controls, in a case-control dataset.
     \item 
       New argument \code{relative} determines whether to compute
       probabilities or relative risks.
    \end{itemize}
...
\item New argument \texttt{unitname} makes it possible to change the name of
  ...
  \item \texttt{residuals.ppm}:
	...
	of class \texttt{msr}. It can be plotted directly.
      ...
      \item \texttt{rgb2hex}, \texttt{col2hex}, \texttt{paletteindex},
        \texttt{is.colour}, \texttt{samecolour}, \texttt{complementarycolour},
        \texttt{is.grey}, \texttt{to.grey}:
        Now handle transparent colours.
 ...
 \item \texttt{rmh.default}:
  ...
  \item \texttt{rNeymanScott}:
   ...
   \item \texttt{rLGCP}:
     \begin{itemize}
     \item 
       No longer requires \pkg{RandomFields} to be loaded explicitly.
     \item 
     New argument \texttt{nsim} allows multiple patterns to be generated.
   \end{itemize}
   ...
   \item \texttt{rpoislinetess}:
     Return value now has an attribute \texttt{lines} giving the 
     realisation of the Poisson line process.

   \item \texttt{rpoispp}:
     \begin{itemize}
     \item 
    New argument \texttt{ex} makes it possible to generate a random pattern
    similar to an example point pattern.
  \item Accelerated when \texttt{lambda} is an image.
     \end{itemize}
  ...
  \item \texttt{rSSI}:
   ...
   When the model is approximately Poisson, it is simulated using 
   \texttt{rpoispp}. 
  ...
  \item \texttt{rVarGamma}: Accelerated.
 ...
 \item \texttt{Softcore}:
  ...
  \item \texttt{solutionset}:
    The expression will be evaluated using pixel arithmetic (Math.im)
   if it cannot be evaluated using \texttt{eval.im}. 
   
  \item \texttt{spatstat.options}:
    \begin{itemize}
    \item New options \texttt{fastthin} and \texttt{fastpois}.
 ...
 \item \texttt{split.ppp}: 
   \begin{itemize}
   \item The splitting variable \texttt{f} can now be a logical vector.
   \item 
   New argument \texttt{reduce}.
   \item
   Result now inherits class \texttt{"ppplist"} and \texttt{"solist"}, 
   as well as \texttt{"splitppp"}.
   \end{itemize}
   
 \item \texttt{split.ppx}:   Result is now of class 'anylist'.
 ...
 \item \texttt{square}
  ...
  \item \texttt{summary.ppm}:
   ...
   (objects of class \texttt{"ppplist"} or \texttt{"splitppp"})
   ...
   \item \texttt{tess}:
    New argument \texttt{unitname} specifies the name of the unit of length.
   ...
   \item \texttt{to.grey}:
     \begin{itemize}
     \item 
        Now uses better weights for the \texttt{R, G, B} channels. 
     \item 
 ...
 \item \texttt{update.kppm}: Now handles additional arguments in any order, 
   with or without names. Changed arguments. Improved behaviour. 
 ...
 \item \verb![.fv!
  ...
  \item \verb![.hyperframe!: 
    When a row of data is extracted with \texttt{drop=TRUE}, the result
   belongs to class \texttt{"anylist"}.
  ...
  \item \verb![.pp3!, \verb![.lpp!, \verb![.ppx!, 
    \texttt{subset.ppp}, \texttt{subset.pp3}, \texttt{subset.lpp}, 
    \texttt{subset.ppx}:
