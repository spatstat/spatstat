...
\item \texttt{Math.linim}: arithmetic for pixel images on a linear network.
  ...
  \item \texttt{dirichlet.network}, \texttt{delaunay.network}:
   The linear networks formed by the Dirichlet tessellation
   and Delaunay triangulation.
  \item \texttt{dirichlet.edges}: the edges of the Dirichlet tessellation.
\item \texttt{quantess}:
...
\item \texttt{dirichlet.vertices}: Vertices of the Dirichlet tessellation.
