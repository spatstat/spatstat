...
mergeSizeTables <- function(a, b) {
  ...
  if(any(after <- (b$date > max(a$date)))) {
    #' some entries in 'b' are later than the last entry in 'a'
    lastA <- a[nrow(a), ,drop=FALSE]
    newB <- b[after, , drop=FALSE]
    newB[,counts] <- newB[,counts] + lastA[,counts]
    newB$version <- lastA$version
    a <- rbind(a, newB)
  }
  return(a)
}
...
if(isTRUE(getSpatstatVariable("Spatstat.Is.Umbrella"))) {
  zgeom <- getSizeTable("spatstat.geom")
  z <- mergeSizeTables(z, zgeom)
  zcore <- getSizeTable("spatstat.core")
  z <- mergeSizeTables(z, zcore)
} 
...
The \spst\ package has grown by \Sexpr{growth}\% 
...
\pkg{spatstat} is being split into sub-packages, to satisfy
the requirements of CRAN. 
    
%#%^!else
\section{\pkg{spatstat} has been split into parts}

The original \pkg{spatstat} package has been split into sub-packages, to satisfy
the requirements of CRAN. 
%#%^!endif
...
\subsection{The parts of \pkg{spatstat}}
...
%#%^!endif
    ...
    \begin{itemize}
    \item 
      The \code{spatstat} package contains
%#%^!ifdef ANTEDILUVIAN
      the main code.
%#%^!else
...
%#%^!ifndef ANTEDILUVIAN
    ...
    \item
      ...
      contains the main code for statistical analysis of spatial point patterns
      and other spatial data.
%#%^!endif
    \end{itemize}
    
    Typing \code{library(spatstat)} 
    will automatically load 
%#%^!ifdef ANTEDILUVIAN
    \pkg{spatstat.data}.
%#%^!else
    \pkg{spatstat.data}, \pkg{spatstat.geom} and \pkg{spatstat.core}.
%#%^!endif 
    
    Typing \code{library(spatstat)} also \textbf{``imports''}
    \pkg{spatstat.utils} and \pkg{spatstat.sparse} (without reporting this).
    ...
    \item \pkg{spatstat.Knet} for analysing point patterns
      on a network.
...
%#%^!ifndef ANTEDILUVIAN
  \item \spst\ now requires the packages \pkg{spatstat.core} 
    and \pkg{spatstat.geom}.
%#%^!endif

  \item The interactive graphics functions \texttt{iplot} and \texttt{istat}
    have been removed from \spst\ into a new package \pkg{spatstat.gui}.
  ...
  \item New dataset \texttt{concrete}.
...
\item \texttt{adaptive.density}:
  ...
  The calculations are performed by either \texttt{densityVoronoi} or
  \texttt{densityAdaptiveKernel}.
...
%%I
 ...
 \item \texttt{integral.im}:
   Accelerated in the case where \texttt{domain} is a tessellation.
