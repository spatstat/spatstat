...
%  $Revision: 1.37 $ $Date: 2026/05/18 06:38:53 $
...
\item \texttt{nnmark, as.im.ssf}:
 ...
 \begin{itemize}
 \item \texttt{MinkowskiSum}, operator \verb!%(+)%!:
   Result was often incorrect for non-convex sets
    (and was not symmetric in \texttt{A} and \texttt{B}).
    \bugbefore{spatstat 1.65-0}{december 2020}{spatstat.geom 3.7-2}{march 2026} 
 ...
 \item \texttt{rLGCP}, \texttt{simulate.kppm}:
  ...
  \item \texttt{kppm}: 
    Results were sometimes incorrect for \texttt{method='clik2'}
    and \texttt{method='palm'}
...
\item \texttt{envelope.ppm}:
 ...
 \item \texttt{update.kppm}:
  ...
  \begin{itemize}
  \item \texttt{rebound.owin} (Undocumented internal function):
    For binary mask windows, the pixel coordinates of the input and output
    were not aligned, even when the new rectangle was the same as the old one.
    \bugalways{spatstat 1.42-0}{may 2015}{spatstat.geom 3.8-2}{july 2026}
    
  \item \texttt{spatstat.sparse} (numerous calculations):
    Results were sometimes incorrect for symmetric sparse matrices
    inheriting class \texttt{"symmetricMatrix"}.
    \bugin{spatstat.sparse 1.0-0}{may 2020}{spatstat.sparse 3.2-0}{may 2026}    
   
  \item \texttt{fitted.slrm}:
    Calculation was incorrect when \texttt{leaveoneout=TRUE}
    and \texttt{type != "intensity"}.
    \bugbefore{spatstat 1.65-0}{december 2020}{spatstat.model 3.5-0}{november 2025}    
    
  \item \texttt{Kmark}:
    Calculation was incorrect for multitype patterns (categorical marks).
    \bugbefore{spatstat 1.41-0}{january 2015}{spatstat.explore 3.6-0}{november 2025}
...
\item \texttt{hyperframe}:
 ...
 %% LEVEL 4: 
 \subsection{Partially Incorrect or Having Only Minor Impact}
 \begin{itemize}
 \item \texttt{dilation.owin}: 
    For very small positive distances \texttt{r > 0} the result did not always
    contain the input region \texttt{w} (and was empty in some extreme cases).
    \bugbefore{spatstat 1.42-0}{may 2015}{spatstat.geom 3.8-2}{july 2026}
    
  \item \texttt{intersect.lintess}:
    Result was incorrect if \texttt{X}
    was a tessellation and \texttt{Y} was a linear network.
    \bugbefore{spatstat 1.46-0}{july 2016}{spatstat.linnet 3.3-3}{november 2025}    
 \item \texttt{incircle}:
   Gave incorrect results for some irregular windows.
   \bugin{spatstat 1.14-4}{october 2008}{spatstat.explore 3.6-0}{november 2025}   
