...
Bugs which cause an error exit are not listed, because these 
...
\item \texttt{hyperframe}:
 ...
 \item \texttt{mppm}:
   Internal data were malformed if the interaction was \texttt{Hardcore}
    ...
    \bugger{1.61-0}{september 2019}{1.64-0}{march 2020}.

 \item \texttt{mppm}:
   Ignored the arguments \texttt{nd} and \texttt{eps} controlling the 
   quadrature scheme. 
    \bugger{1.35-0}{december 2013}{1.64-0}{march 2020}.
