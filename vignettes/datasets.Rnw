...
{\tt chicago} & street crimes & point pattern on linear network \\
...
\SweaveOpts{width=12,height=4}\setkeys{Gin}{width=\textwidth}
<<fig=TRUE>>=
...
\subsubsection*{\texttt{chicago}: Chicago street crimes}

Street crimes reported in a two-week period
...
\subsubsection*{\texttt{redwood}, \texttt{redwoodfull}: Redwood seedlings and saplings}
...
been used as a demonstration example.
