...
type \texttt{plot({\em name})}. A nicer plot
will often be produced by typing \texttt{example(\emph{name})}.
...
Additional information about a point pattern data set \texttt{\em name}
may be stored in a separate list \texttt{{\em name}.extra}.
Currently these are the auxiliary data sets:
...
Additionally, raw data files are available for the datasets 
\texttt{amacrine}, \texttt{finpines}, \texttt{gorillas},
\texttt{vesicles} and \texttt{gorillas}.
See the help files for these datasets for information about the
raw data files.

\subsection{Other Spatial Data Types}

There are also the following spatial data sets which are not 2D point patterns:

\begin{tabular}[c]{l|l|l|l}
{\sf name} & {\sf description} & {\sf class} & {\sf format} \\ \hline
{\tt austates} & Australian states & \texttt{tess} & tessellation \\
{\tt cetaceans} & marine survey & \texttt{hyperframe} & replicated 2D point patterns \\
{\tt chicago} & crimes & \texttt{lpp} & point pattern on linear network \\
{\tt demohyper} & simulated data & \texttt{hyperframe} & replicated 2D point patterns with covariates\\
{\tt dendrite} & dendritic spines & \texttt{lpp} & point pattern on linear network \\
{\tt flu} & virus proteins & \texttt{hyperframe} & replicated 2D point patterns \\
{\tt heather} & heather mosaic & \texttt{solist} & binary image (three versions) \\
{\tt osteo} & osteocyte lacunae & \texttt{hyperframe} & replicated 3D point patterns with covariates\\
{\tt pyramidal} & pyramidal neurons & \texttt{hyperframe} & replicated 2D point patterns in 3 groups\\
{\tt residualspaper} & data \& code & \texttt{list} & 2D point patterns, \R\ function \\
{\tt simba} & simulated data & \texttt{hyperframe} & replicated 2D point patterns in 2 groups\\
{\tt spiders} & spider webs & \texttt{lpp} & point pattern on linear network \\
{\tt waterstriders} & insects on water & \texttt{solist} & replicated 2D point patterns\\
\hline
\end{tabular}
...
Raw data files are available for the \texttt{osteo} dataset.
See the help for \texttt{osteo} for information.
