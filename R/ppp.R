...
#
#	$Revision: 4.89 $	$Date: 2014/02/07 08:58:53 $
#
      ...
      if(is.data.frame(mks)) {
        ...
        if(is.factor(mks)) {
          ...
          if(inherits(mks, "Date")) {
            desc <- paste("dates, of class", sQuote("Date"))
          } else if(inherits(mks, "POSIXt")) {
            desc <- paste("dates, of class", sQuote("POSIXt"))
          } else {
            desc <- paste0(if(is.numeric(mks)) "numeric, ",
                           "of storage type ", sQuote(typeof(mks)))
            styp <- typeof(mks)
          }
          cat(paste("marks are", desc, "\n"))
        }
      }
    }
    print(x$window)
...
print.summary.ppp <- function(x, ..., dp=3) {
  ...
  if(!is.null(nrejects <- x$rejects)) 
    cat(paste("\n***",
              ...
              paste("attr(,", dQuote("rejects"), ")", sep=""),
              "***\n"))
  if(!is.null(info <- x$rmhinfo)) {
    cat("\nPattern was generated by Metropolis-Hastings algorithm rmh")
    print(info)
  }
  return(invisible(x))
}
