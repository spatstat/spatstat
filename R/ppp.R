...
#
#	$Revision: 4.96 $	$Date: 2014/02/27 06:32:02 $
#
...
print.summary.ppp <- function(x, ..., dp=3) {
  ...
  cat(if(x$is.marked) "Marked planar" else "Planar",
      "point pattern: ",
      x$n,
      "points",
      fill=TRUE)
  ...
  if((!is.null(ndup)) && (ndup > 0))
    cat("\n*Pattern contains", "duplicated points*", fill=TRUE)
  rndg <- x$rounding
  if(!is.null(rndg)) {
    cat("\n")
    if(rndg >= 1) {
  ...
  if(x$is.marked) {
    if(x$multiple.marks) {
      mn <- x$marknames
      mn <- commasep(mn, ", ", flatten=FALSE)
      cat("Mark variables:", mn, fill=TRUE)
      cat("Summary:\n")
      print(x$marks)
