...
and now contains almost 100,000 lines of code.
...
\item \texttt{kstest.mppm}:
  Kolmogorov-Smirnov test of goodness-of-fit for a model of class \texttt{mppm}
...
\item \texttt{simplepanel}, \texttt{run.simplepanel}:
  ...
  \item \texttt{kstest.lpp}, \texttt{kstest.lppm}:
  ...
  \item \texttt{bermantest.lpp}, \texttt{bermantest.lppm}:
...
\item \texttt{lohboot}:
  ...
  \item \texttt{lppm}:
   New arguments \texttt{eps} and \texttt{nd} control the quadrature scheme.
   Stepwise model selection using \texttt{step} now works for lppm objects.
...
\item \texttt{plot.hyperframe}: 
 ...
 \item 
  ...
  \item 
    ...
    \item  
      New arguments \code{add} and \code{do.plot}.
    \item 
      More arguments recognised by \code{ribargs}
    \end{itemize}
  
  \item \texttt{plot.kstest}:
  ...
  \item \texttt{plot.ppp}:
    ...
    \item 
      New arguments \code{show.window} and \code{show.all}
   \end{itemize}
