...
\pkg{spatstat} versions \texttt{1.18-0} and above.
...
We'll use three other packages to handle shapefile data.
...
The \pkg{gpclib} package supports geometrical operations 
on polygons --- such as union and intersection. 
However, \pkg{gpclib} has a restrictive licence which 
forbids its use for commercial activities. 
Type \texttt{licence.polygons()} for more information.
\underline{
  {\bf By default,} \pkg{gpclib} {\bf is disabled. It must be
  enabled }
}
by typing \verb!spatstat.options(gpclib=TRUE)! assuming you are
