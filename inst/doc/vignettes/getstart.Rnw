...
\item
  You are airborne!
  Now look at the workshop notes \cite{badd10wshop} for more hints.
\end{enumerate}
...
\item online help:
  \begin{quote}
    ...
    Alternatively you can download a pdf of the entire manual (800 pages!)
    from the website \texttt{www.spatstat.org}. 
  \end{quote}
\item workshop notes:
  \begin{quote}
    A complete set of notes from an Introductory Workshop is available
    from \texttt{www.csiro.au/resources/pf16h.html}
    or by visiting \texttt{www.spatstat.org}
  \end{quote}
\item vignettes:
  \begin{quote}
...
% The following is inserted from refs.bbl originally generated from refs.tex
